        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '48px' }} className="contact-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div className="glass-card" style={{ padding: '32px' }}>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#03252A', marginBottom: '20px' }}>
                Instant Support & Helpline
              </h3>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', fontSize: '0.95rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'rgba(0, 122, 135, 0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#007A87' }}>
                    <Phone size={20} />
                  </div>
                  <div>
                    <div style={{ fontSize: '0.75rem', color: '#64748B' }}>Customer Care Phone</div>
                    <strong style={{ color: '#03252A' }}>{adminData?.siteSettings?.supportPhone || '+91 98765 43210'}</strong>
                  </div>
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'rgba(3, 37, 42, 0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#03252A' }}>
                    <Mail size={20} />
                  </div>
                  <div>
                    <div style={{ fontSize: '0.75rem', color: '#64748B' }}>Email Inquiry</div>
                    <strong style={{ color: '#03252A' }}>{adminData?.siteSettings?.supportEmail || 'support@nexdoorclean.com'}</strong>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  ...
                  <div>
                    ...
                    <strong style={{ color: '#03252A' }}>{adminData?.siteSettings?.operatingHours || 'Monday - Sunday: 07:00 AM - 09:00 PM'}</strong>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Hubs */}
            <div className="glass-card" style={{ padding: '32px' }}>
              <h4 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#03252A', marginBottom: '16px' }}>
                ...
                {adminData?.locations && adminData.locations.filter(l => l.status === 'active').length > 0 ? (
                  adminData.locations.filter(l => l.status === 'active').map((loc) => (
                    <li key={loc.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      <MapPin size={18} style={{ color: '#007A87', flexShrink: 0, marginTop: '2px' }} />
                      <div>
                        <strong style={{ color: '#03252A', display: 'block' }}>{loc.name} Hub</strong>
                  ...
                  <>
                    <li style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      <MapPin size={18} style={{ color: '#007A87', flexShrink: 0, marginTop: '2px' }} />
                      <div>
                        <strong style={{ color: '#03252A', display: 'block' }}>Kakkanad HQ Hub</strong>
                    ...
                    <li style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      <MapPin size={18} style={{ color: '#007A87', flexShrink: 0, marginTop: '2px' }} />
                      <div>
                        <strong style={{ color: '#03252A', display: 'block' }}>Edappally Service Fleet</strong>
                    ...
                    <li style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      <MapPin size={18} style={{ color: '#007A87', flexShrink: 0, marginTop: '2px' }} />
                      <div>
                        <strong style={{ color: '#03252A', display: 'block' }}>Kalamassery Unit</strong>
          ...
          <div className="glass-card" style={{ padding: '40px', borderRadius: '24px' }}>
            {submitted ? (
              <div style={{ textAlign: 'center', padding: '40px 0' }}>
                <CheckCircle2 size={48} style={{ color: '#9BC83B', margin: '0 auto 16px auto' }} />
                <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#03252A' }}>Inquiry Submitted!</h3>
                {inquiryRefId && (
                  <div style={{ background: 'rgba(0, 122, 135, 0.1)', padding: '8px 16px', borderRadius: '12px', display: 'inline-block', margin: '12px 0', color: '#007A87', fontWeight: 800, fontSize: '0.9rem' }}>
